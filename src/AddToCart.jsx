    ...
    return (
        <>
            {cartSelector.some((cartitem) => cartitem.id === props.product.id) ? ( 
                <button className="add-to-cart test" onClick={() => dispatch(removeitem(props.product))}>Remove from Cart</button>
            ) : (
                <button className="add-to-cart t" onClick={() => dispatch(additem(props.product))}>Add to Cart </button>
            )
            }
        </>
    );
